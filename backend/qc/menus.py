...
from .shim.sixshim import text_type as unicode
...
from .shim.utils.config import JSONConfig
...
PLUGIN_MENUS = OrderedDict([
       ...
       # ---- 以下 AZW3 / PDF / TXT 检查是本仓新增的（上游 Quality Check 没有这些项）----
       # AZW3 那四项挂在「MOBI / AZW3」组里（cat=mobi），但只判 AZW3 文件本身。
       ('check_azw3_drm',           {'name': _('Check AZW3 DRM'),                 'cat':'mobi',     'sub_menu': _('Check Mobi'),     'group': 2, 'excludable': True,  'image': 'images/check_mobi_asin.png',           'tooltip':_('Find AZW3 formats that are DRM protected (PalmDOC encryption flag)')}),
       ('check_azw3_truncated',     {'name': _('Check AZW3 record table'),        'cat':'mobi',     'sub_menu': _('Check Mobi'),     'group': 2, 'excludable': True,  'image': 'images/check_mobi_asin.png',           'tooltip':_('Find AZW3 formats whose record table points past the end of the file')}),
       ('check_azw3_missing_thumb', {'name': _('Check AZW3 cover records'),       'cat':'mobi',     'sub_menu': _('Check Mobi'),     'group': 3, 'excludable': True,  'image': 'images/check_covers.png',              'tooltip':_('Find AZW3 formats with no cover or thumbnail records (EXTH 201/202/203)')}),
       ('check_azw3_mobi6_only',    {'name': _('Check AZW3 KF8 layer'),           'cat':'mobi',     'sub_menu': _('Check Mobi'),     'group': 3, 'excludable': True,  'image': 'images/check_mobi_asin.png',           'tooltip':_('Find AZW3 formats that only carry a MOBI6 header instead of KF8')}),

       ('check_pdf_unreadable',     {'name': _('Check unreadable PDF'),           'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 0, 'excludable': True,  'image': 'images/check_epub.png',                'tooltip':_('Find PDF formats that cannot be opened, or that report no pages')}),
       ('check_pdf_encrypted',      {'name': _('Check encrypted PDF'),            'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 0, 'excludable': True,  'image': 'images/check_epub_encryption.png',     'tooltip':_('Find PDF formats that are encrypted, or whose permissions are restricted')}),
       ('check_pdf_no_text_layer',  {'name': _('Check PDF text layer'),           'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 1, 'excludable': True,  'image': 'images/check_epub.png',                'tooltip':_('Find PDF formats with no text layer (scanned images), which cannot be searched, read aloud or converted')}),
       ('check_pdf_mixed_page_size',{'name': _('Check PDF page sizes'),           'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 1, 'excludable': True,  'image': 'images/check_epub.png',                'tooltip':_('Find PDF formats that mix different page sizes')}),
       ('check_pdf_oversized',      {'name': _('Check PDF size per page'),        'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 2, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find PDF formats that are unusually heavy per page (uncompressed images, high DPI)')}),
       ('check_pdf_no_toc',         {'name': _('Check PDF table of contents'),    'cat':'pdf',      'sub_menu': _('Check PDF'),      'group': 2, 'excludable': True,  'image': 'images/check_epub_toc.png',            'tooltip':_('Find PDF formats with no extractable table of contents')}),

       ('check_txt_empty',          {'name': _('Check empty TXT'),                'cat':'txt',      'sub_menu': _('Check Text'),     'group': 0, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats with no content at all')}),
       ('check_txt_not_utf8',       {'name': _('Check TXT encoding'),             'cat':'txt',      'sub_menu': _('Check Text'),     'group': 0, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats that are not valid UTF-8 (GBK, Big5, UTF-16 and friends)')}),
       ('check_txt_bom',            {'name': _('Check TXT byte order mark'),      'cat':'txt',      'sub_menu': _('Check Text'),     'group': 1, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats that start with a byte order mark')}),
       ('check_txt_control_chars',  {'name': _('Check TXT control characters'),   'cat':'txt',      'sub_menu': _('Check Text'),     'group': 1, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats containing control characters, NUL bytes in particular')}),
       ('check_txt_mixed_newlines', {'name': _('Check TXT line endings'),         'cat':'txt',      'sub_menu': _('Check Text'),     'group': 2, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats that mix CRLF, LF and CR line endings')}),
       ('check_txt_long_lines',     {'name': _('Check TXT line length'),          'cat':'txt',      'sub_menu': _('Check Text'),     'group': 2, 'excludable': True,  'image': 'images/check_book.png',                'tooltip':_('Find TXT formats with over-long lines, or with no line break at all')}),
